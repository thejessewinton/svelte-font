...
import { svelteFontError } from '../next-font-error';
...
export function validateLocalFontFunctionCall(
  ...
  if (functionName) {
    svelteFontError(`next/font/local has no named exports`);
  }
  let {
    src,
    display = 'swap',
  ...
  if (!src) {
    svelteFontError('Missing required `src` property');
  }
  ...
  if (!Array.isArray(src)) {
    ...
    if (src.length === 0) {
      svelteFontError('Unexpected empty `src` array.');
    }
  }
  ...
  src = src.map((fontFile: any) => {
    ...
    if (!ext) {
      svelteFontError(`Unexpected file \`${fontFile.path}\``);
    }
  ...
  if (Array.isArray(declarations)) {
    declarations.forEach((declaration) => {
      if (
        ...
        svelteFontError(`Invalid declaration prop: \`${declaration.prop}\``);
      }
    });
  }
