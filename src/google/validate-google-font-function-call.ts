...
export function validateGoogleFontFunctionCall(
  ...
  if (functionName === '') {
    svelteFontError(`next/font/google has no default export`);
  }
  ...
  if (!fontFamilyData) {
    svelteFontError(`Unknown font \`${fontFamily}\``);
  }
  ...
  if (weights[0] !== 'variable' && axes) {
    svelteFontError('Axes can only be defined for variable fonts');
  }
